...
export default NotifyList = ({ navigation }) => {
    ...
    const adocao = (animalid, sender, reciever, id) => {
        ...
        updateDoc(doc(collection(config.db, "animais"), animalid), {
            ...
            Alert.alert("Transferido", "O animal foi transferido com sucesso!");
            deleteNotification(id);
        });
        // apaga o chat
        deleteDoc(doc(config.db, "chats", id));
        // avisa o novo dono do animal
        sendPushNotification("Adotado", `${user.nome_perfil} concordou com a adoção, parabéns!`, user.uid, sender, animalid)
            .then(()=>{
